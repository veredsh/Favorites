/* low_then.asm  
 *low between N numbers: X1<X2< ... <Xn return #t or #f
 *args = 1 or more 
 *special (< 5) -> #t
 */
 ...
 LOW_THEN_LOOP:
	...
	JUMP_EQ(CHECK_OTHER_NUM_LOW_THEN);
	SHOW("The argumen in the R2  is not integer. print R2",IND(R2));
	HALT;
 CHECK_OTHER_NUM_LOW_THEN:	
	CMP(IND(R3),T_INTEGER); /*CHECK IF THE ARGUMENT IN R3  IS INTEGER*/
	JUMP_EQ(ARG_IS_OK_LOW_THEN);
	SHOW("The argumen in the R3  is not integer. print R3",IND(R3));
	HALT;
 ARG_IS_OK_LOW_THEN:	
	CMP(INDD(R2,1),INDD(R3,1)); /*COMPERE THE FIRST ARG VALUE WITH THE NEXT ONE NEED TO BE LOWER*/
	JUMP_GE(ANS_FALSE_LOW_THEN);
	INCR(R1);
	JUMP(LOW_THEN_LOOP);
 LOW_THEN_LOOP_END:
